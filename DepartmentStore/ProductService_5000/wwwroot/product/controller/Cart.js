...
function RenderCartTable() {
    $.ajax({
            ...
            let tableBody = response.length === 0
                ...
                : response.map((cart) => `
                    <tr class="text-center">
									...
									<td class="price-cart" id="signle_price_cart">${cart.price.toLocaleString('vi-VN') }</td>
        ...
        error: function () {
            $('#div_table_cart').html('<tr><td colspan="8" class="text-center text-danger">Không thể lấy dữ liệu gAAAAAAiỏ hàng</td></tr>');
        }
    });
}
